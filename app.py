...
def main():
    ...
    st.markdown("""
        <div style='background-color: #2E8B57; padding: 1rem; border-radius: 0.5rem; margin-bottom: 2rem;'>
    ...
    with st.sidebar:
        ...
        # )
        ngrok_url = st.text_input("LLM Remote URL", help="Enter your NGROK URL")
        ngrok_username = st.text_input("LLM Server Username")
        ngrok_password = st.text_input("LLM Server Password", type="password")

        # if not llm_api_key:
        #     st.warning("⚠️ Please enter your LLM API Key to proceed")
        #     st.markdown("[Get your API key here](https://aistudio.google.com/apikey)")
        #     return
        if ngrok_username and ngrok_password and ngrok_url:
            client = Client(host=ngrok_url, auth=(ngrok_username, ngrok_password))
        else:
            client = None
